...
/*int getLabelValue(map<string, int> &symTbl, string label){
    map<string, int>::iterator it = symTbl.find(label);
    if (it != symTbl.end()) {
        return it->second;
    }
        ...
        for(it = tokLines.begin(); it != tokLines.end(); ++it){
            ...
            for(it2 = it->begin(); it2 != it->end(); ++it2){
                ...
                if (prevToken == ""){
                    ...
                    if ((curToken != "INT") && (curToken != "HEXINT") && (curToken != "ID")) {
        ...
        for (it = tokLines.begin(); it != tokLines.end(); ++it) {
            ...
            for (it2 = it->begin(); it2 != it->end(); ++it2) {
                ...
                if (prevToken == ""){
                    ...
                    //PASS two we don;t need to do anything here..
                    }else if (curToken == "LABEL"){
                        prevToken = "";
                    }
                    ...
                    int word;
                    if (curToken == "ID"){
                        //get labels string and look up value to replace with in symbol table
                        ...
                        //word must be a hex or int already
                        word = (*it2)->toInt();
                    }
                    output_word(word);
                    prevToken = "";
